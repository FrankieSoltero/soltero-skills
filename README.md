...
| `token-economy` | Usage-limit economy from measured transcripts: a bundled audit ranks the levers by uncached input (hook-spawned headless sessions, cache-TTL breaks, compactions, unpinned dispatches, whole-file reads); a bundled idempotent setup installs the system at user scope (calibrated context-watch hook, effort, marker-delimited protocol block in `~/.claude/CLAUDE.md`) with backups. |
...
| `instagram-studio` | Code project or brief → Instagram Reels, Stories, 4:5 feed video, and carousels rendered locally with Hyperframes, gated by a preflight check, a facts.md claim source, and a claim-traced caption. |
