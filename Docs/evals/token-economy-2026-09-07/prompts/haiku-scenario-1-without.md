...
Your user-scope config directory for this task is /tmp/te-haiku-scenario-1-without/.claude — treat it exactly
as you would ~/.claude (settings.json, CLAUDE.md, hooks/, and projects/ with the session
transcripts are all there; pass /tmp/te-haiku-scenario-1-without wherever a tool wants your home directory).
Apart from the skills directory named above, do not read or write anything outside
/tmp/te-haiku-scenario-1-without.
...
Produce two things. (1) /tmp/te-haiku-scenario-1-without/usage-findings.md: what is consuming the budget,
ranked, with the numbers you based each item on and where each number came from. (2) Whatever
configuration changes you decide to make under /tmp/te-haiku-scenario-1-without/.claude, applied. In your
