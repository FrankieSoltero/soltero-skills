...
Your user-scope config directory for this task is /tmp/te-sonnet-scenario-1-with/.claude — treat it exactly
as you would ~/.claude (settings.json, CLAUDE.md, hooks/, and projects/ with the session
transcripts are all there; pass /tmp/te-sonnet-scenario-1-with wherever a tool wants your home directory).
...
Produce two things. (1) /tmp/te-sonnet-scenario-1-with/usage-findings.md: what is consuming the budget,
ranked, with the numbers you based each item on and where each number came from. (2) Whatever
configuration changes you decide to make under /tmp/te-sonnet-scenario-1-with/.claude, applied. In your
