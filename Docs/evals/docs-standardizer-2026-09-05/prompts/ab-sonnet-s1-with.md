...
Your working directory is /tmp/acme-ledger-ab-sonnet-s1-with. Your user-scope config directory for this task
is /tmp/acme-home-ab-sonnet-s1-with/.claude — treat it exactly as you would ~/.claude (read it, write there
if something belongs at user scope, and pass /tmp/acme-home-ab-sonnet-s1-with wherever a tool wants your home
directory). Apart from the skills directory named above, do not
read or write anything outside /tmp/acme-ledger-ab-sonnet-s1-with and /tmp/acme-home-ab-sonnet-s1-with.

/tmp/acme-ledger-ab-sonnet-s1-with is a Node ledger service. A new engineer starts Monday, and the agent the
