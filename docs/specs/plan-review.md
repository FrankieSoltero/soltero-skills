...
- **Scope / non-goals:** Convene a 6-grader council via bundled Workflow
  ...
  confirmed miss → re-grade, script records min. Gate: overall ≥95 AND every dimension
  ≥80, else BLOCKED — no execution, no "safe first tasks", no partial start. Report to
...
- **Success scenario:** Given a flawed plan (vague tasks, tests-at-the-end, an
  ...
  dropped, deploy-before-auth ordering), the council scores it far below 95, stamps
