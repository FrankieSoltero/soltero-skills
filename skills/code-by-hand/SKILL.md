---
name: code-by-hand
description: Use ONLY when the user explicitly invokes it ("code-by-hand", "let's build this by hand", "slow mode — I type everything"). Never auto-triggers — no task shape, learning goal, or hunch activates this mode without the user asking for it by name. Navigator/driver pairing where the USER types every line of code — the agent plans the change, presents one logical block at a time (5-15 lines, why-this-block plus per-line notes, builder-to-builder, no quizzes), waits while the user types it into the real file, then READS the file and verifies what was actually typed — typos get explained and fixed by the user's own hands, working-but-different versions get trade-offs explained and the user decides. The agent never writes code files during a session (typos, imports, and deadlines included); its one writable file is the .code-by-hand.md session log. Checkpoint compiler/test runs at natural boundaries; mode exit only by explicit user say-so. For understanding an existing diff, use walkthrough-tutor instead.
---
