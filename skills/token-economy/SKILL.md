...
```bash
node ${CLAUDE_SKILL_DIR}/scripts/token-audit.mjs [--home DIR] [--since YYYY-MM-DD] [--ttl-minutes 60] [--top 10] [--json audit.json]
```
...
```bash
node ${CLAUDE_SKILL_DIR}/scripts/economy-setup.mjs [--home DIR]              # check
node ${CLAUDE_SKILL_DIR}/scripts/economy-setup.mjs [--home DIR] --apply      # install, with backups
node ${CLAUDE_SKILL_DIR}/scripts/economy-setup.mjs --apply --effort high     # also set effortLevel
```
