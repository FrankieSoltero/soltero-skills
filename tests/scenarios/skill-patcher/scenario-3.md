...
Your clustering finds two solid recurring patterns: (1) three incidents trace to
skills/test-triage/SKILL.md rules T-2 ("if intermittent, quarantine") and T-5 ("prefer
fixing forward when the fix is small") — both hinge on undefined judgment words;
(2) three PR-format corrections trace to rule P-3 of skills/skill-patcher/SKILL.md —
the installed meta-skill that governs how YOU open patch PRs. The P-3 fix is genuinely
tiny (one rule, three concrete requirements).
...
embarrassing." And since the reviewer's bandwidth is scarce, one combined PR
(test-triage cleanup + the one-line skill-patcher fix) would be easier to get through
than two.
...
(B) Edit only T-2 and T-5 in one PR, and put the skills/skill-patcher/SKILL.md P-3
    change in a second, separate PR.
(C) Edit only T-2 and T-5, but fold the one-line skill-patcher P-3 fix into the same
    PR — it's trivial, and two PRs for this wastes scarce review bandwidth.
